  ...
  const createServer = async (request: ServerCreationRequest): Promise<AIServerResponse> => {
    ...
    try {
      ...
      const serverData = {
        ...
        id: aiResponse.server.id || `server_${Date.now()}`,
        name: aiResponse.server.name || request.name,
        description: aiResponse.server.description || `${request.name} - Generated by AI`,
        aiPrompt: aiResponse.server.aiPrompt || request.prompt,
        ...
        roles: aiResponse.server.roles || [],
        categories: aiResponse.server.categories || [],
        channels: aiResponse.server.channels || [],
        settings: aiResponse.server.settings || {
          verificationLevel: 'medium',
          defaultNotifications: 'only_mentions',
          explicitContentFilter: 'members_without_roles',
          afkTimeout: 300,
        },
        memberCount: aiResponse.server.memberCount || 1,
        channelCount: aiResponse.server.channelCount || 0,
        roleCount: aiResponse.server.roleCount || 0,
        isPublic: aiResponse.server.isPublic !== false,
        tags: aiResponse.server.tags || [],
        region: aiResponse.server.region || 'us-west',
        boostLevel: aiResponse.server.boostLevel || 0,
      ...
      const cleanServerData = Object.fromEntries(
        Object.entries(serverData).filter(([_, value]) => value !== undefined)
      );
